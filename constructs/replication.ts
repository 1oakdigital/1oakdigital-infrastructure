  ...
  constructor(stack: string, props: DmsReplicationProps) {
    ...
    const dbMap = {
      admin: {
        ...
        password: config.getSecret("db-admin-password"),
        sites: ["admin"],
      },
      "db-live-1": {
        serverName: "db-live-1-do-user-7412958-0.b.db.ondigitalocean.com",
        password: config.getSecret("db-live-1-password"),
        sites: [
          "yummyaffair",
          "jijuneukt",
          "lustpals",
      ...
      "db-live-3": {
        ...
        password: config.getSecret("db-live-3-password"),
        sites: [
          "flingpals",
          "honeynearby",
          "laissebaiser",
      ...
      "db-live-5": {
        ...
        password: config.getSecret("db-live-5-password"),
        sites: [
          "bangtender",
          "horneyfriends",
          "meltingme",
    ...
    for (const [key, value] of Object.entries(dbMap)) {
      value.sites.forEach((site: string) => {
        ...
        if (props.replicationMapping[site]) {
          ...
          usedSg.push(sgId);
          new aws.dms.ReplicationTask(
            `${stack}-${site}-replication-task`,
            {
